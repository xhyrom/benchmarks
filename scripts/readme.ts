...
for (const [language, benchmarksForLanguage] of Object.entries(benchmarks)) {
    const formattedLanguage = `${language.at(0).toUpperCase() + language.slice(1)}`;
    head += `   - [${formattedLanguage}](#${language})\n`;
    markdown += `## ${formattedLanguage}\n\n`;

    for (const benchmark of benchmarksForLanguage) {
